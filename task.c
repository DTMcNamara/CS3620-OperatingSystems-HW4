...
void* worker_thread_function(void *tinput_void){
    ...
    if(found_solutions==NSOLUTIONS||NSOLUTIONS<tinput->tid){
	return NULL;
    }
    unsigned long first_tried_solution = tinput->tid;
    //1000*1000000000L*1000 is just very big number, which we will never reach
    for(unsigned long attempted_solution=first_tried_solution; attempted_solution<1000*1000000000L*1000; attempted_solution+=NTHREADS){
